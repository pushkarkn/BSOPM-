...
def main():
    ...
    st.table({
        "Current Option Price": [S],
        "Strike Price": [K], "Time to Expiration (Years)": [T], "Risk-Free Interest Rate": [r], "Volatility": [sigma]
    })
    ...
    with col1:
        ...
        st.metric(value=f"${call_price:.2f}")
    with col2:
        st.subheader("PUT Value")
        st.metric(value=f"${put_price:.2f}")
